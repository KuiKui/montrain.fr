...
abstract class BaseMessagePeer {
	...
	const DISCUSSION_ID = 'message.DISCUSSION_ID';
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseMessagePeer', $criteria, $con);
		}
	...
	/**
	 * Returns the number of rows matching criteria, joining the related Discussion table
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct Whether to select only distinct columns; deprecated: use Criteria->setDistinct() instead.
	 * @param      PropelPDO $con
	 ...
	 * @return     int Number of matching rows.
	 */
	public static function doCountJoinDiscussion(Criteria $criteria, $distinct = false, PropelPDO $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		$criteria->addJoin(MessagePeer::DISCUSSION_ID, DiscussionPeer::ID, $join_behavior);
	...
	/**
	 * Selects a collection of Message objects pre-filled with their Discussion objects.
	 * @param      Criteria  $criteria
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	 ...
	 *		 rethrown wrapped into a PropelException.
	 */
	public static function doSelectJoinDiscussion(Criteria $criteria, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$criteria = clone $criteria;
		...
		MessagePeer::addSelectColumns($criteria);
		$startcol = (MessagePeer::NUM_COLUMNS - MessagePeer::NUM_LAZY_LOAD_COLUMNS);
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$key2 = DiscussionPeer::getPrimaryKeyHashFromRow($row, $startcol);
			if ($key2 !== null) {
				$obj2 = DiscussionPeer::getInstanceFromPool($key2);
				if (!$obj2) {
					...
					DiscussionPeer::addInstanceToPool($obj2, $key2);
				} // if obj2 already loaded
				
				// Add the $obj1 (Message) to $obj2 (Discussion)
	...
	public static function doSelectJoinAll(Criteria $criteria, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		DiscussionPeer::addSelectColumns($criteria);
		$startcol3 = $startcol2 + (DiscussionPeer::NUM_COLUMNS - DiscussionPeer::NUM_LAZY_LOAD_COLUMNS);
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			// Add objects for joined Discussion rows

			$key2 = DiscussionPeer::getPrimaryKeyHashFromRow($row, $startcol2);
			if ($key2 !== null) {
				$obj2 = DiscussionPeer::getInstanceFromPool($key2);
				if (!$obj2) {
				...
				// Add the $obj1 (Message) to the collection in $obj2 (Discussion)
				$obj2->addMessage($obj1);
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseMessagePeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseMessagePeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseMessagePeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseMessagePeer', $values, $con, $pk);
    }
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseMessagePeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseMessagePeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseMessagePeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseMessagePeer', $values, $con, $ret);
    }
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseMessagePeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
